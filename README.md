...
Write mode requires the explicit `--apply` flag. It refuses chats without a recoverable checkpoint and uses the normal paginated `TopicMonitor` path. Monitor state is a locked, revisioned atomic file: only a truly absent file initializes to now; corrupt JSON, symlinks, and non-regular files fail closed; a stale writer returns `monitor_state_conflict`; and an AI failure leaves the canonical state unchanged. Catch-up stops the managed LaunchAgent, then must acquire the same menu-app singleton lock before any backup, state, database, or projection write. `menu_app_active` means a manually started app still owns that lock, so catch-up writes no canonical data. With ownership established, it stores a private partial-recovery backup under `~/.we-groupchat-obsidian/backups/monitor-catch-up/`, drains, rebuilds affected source-date indexes and historical Daily Digests, validates SQLite/FTS/hash parity, writes the reconciliation receipt while still holding the lock, releases ownership, and only then restores a previously loaded LaunchAgent.
...
- `complete / drained`: every selected chat reached `no_messages`, projections and canonical validation passed, and the maintenance receipt was durably written under singleton ownership. A previously loaded LaunchAgent is restored after the lock is released; restoration failure makes the command exit nonzero.
...
Receipts contain anonymous monitor-state IDs, checkpoints before/after, page/status counts, canonical event IDs, affected dates, projection counts, validation, backup scope, and the pre-restore LaunchAgent ownership state. They do not copy chat bodies, AI titles, summaries, note paths, or Review Queue content. Non-complete runs return a nonzero exit status; a post-receipt LaunchAgent restoration failure is also reported by a nonzero command exit. The receipt, not an attempted automatic rollback, is the reconciliation authority.
