...
- `complete / drained`: every selected chat reached `no_messages` with `source_eof=true` under an unchanged complete inventory, projections and canonical validation passed, and the maintenance receipt was durably written under singleton ownership. A previously loaded LaunchAgent is restored after the lock is released; restoration failure makes the command exit nonzero.
